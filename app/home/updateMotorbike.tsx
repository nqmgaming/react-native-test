...
import Animated, {FadeIn, FadeInDown, FadeInUp, FadeOutDown, PinwheelIn, StretchInX} from "react-native-reanimated";
const UpdateMotorbikeScreen = () => {
    ...
    const isValidData = (data: Motorbike) => {
        const fields = ['ten_xe_ph31902', 'gia_ban_ph31902', 'mo_ta_ph31902', 'mau_sac_ph31902', 'hinh_anh_ph31902'];
    ...
    return (
        <KeyboardAvoidingView style={styles.container}>
            <Pressable
                ...
                {
                    image ? <Animated.Image entering={FadeOutDown.duration(200).delay(200)} source={{uri: image}} style={{width: 100, height: 100}}/> :
                        (
                           <Image source={motorbike?.hinh_anh_ph31902} style={{width: 100, height: 100}}/>
                        )
                }
            </Pressable>
            {['ten_xe_ph31902', 'gia_ban_ph31902', 'mo_ta_ph31902', 'mau_sac_ph31902'].map((field, index) => (
                <Animated.View entering={FadeInUp.springify().delay(100*(index +1))} key={index}>
                    <TextInput
                        style={styles.input}
                        value={motorbike[field]}
                        onChangeText={(value) => handleChange(field as keyof Motorbike, value)}
                        placeholder={field}
                    />
                </Animated.View>
            ))}
